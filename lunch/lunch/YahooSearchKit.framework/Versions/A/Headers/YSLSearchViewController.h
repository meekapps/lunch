...
#import "YSLTrendingCategories.h"
...
/**
 * This toggles the transparency setting in Web search results.
 * Default is NO.
 *
 * When set to YES, by default, the background is black. If landing page is set, the landing
 * page view will be shown. */
@property (nonatomic, assign, getter = isTransparencyEnabled) BOOL enableTransparency;
...
/**
 * The selected trending category
 *
 * @discussion When set, the search view controller will display the trending results that follows this category.
 * If not set, default category is used, which is YSLTrendingCategoryNews.
 * Set to YSLTrendingCategoryNone if trending results are not desired.
 */
@property (nonatomic, assign) YSLTrendingCategory selectedTrendingCategory;
